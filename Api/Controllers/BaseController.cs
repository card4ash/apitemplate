...
public abstract class BaseController : ControllerBase
{
    ...
    protected readonly IAppUserService UserService;

    protected BaseController(ILogger logger, IAppUserService userService)
    {
        _logger = logger;
        UserService = userService;
    }

    protected async Task<IActionResult> HandleRequest(Func<Task<IActionResult>> func,
    ...
    {
        return httpRequest.HttpContext.User?.Claims?.SingleOrDefault(x => x.Type == "username")?.Value;
    }
}